...
class UpsertBlockTests(SimpleTestCase):
    def test_empty_description_gets_block_only(self):
        ...
        self.assertTrue(out.startswith("Birds seen during activity:"))
        self.assertIn("3 Robin", out)
        self.assertIn("Generated by Roadrunner", out)
        self.assertIn(matching.ROADRUNNER_URL, out)  # clickable link for Strava
        self.assertTrue(out.rstrip().endswith(matching.ROADRUNNER_URL))
        self.assertNotIn("<!--", out)  # no HTML-comment markers in plain-text Strava field
    ...
    def test_existing_text_is_preserved_and_block_appended(self):
        ...
        self.assertIn("Birds seen during activity:", out)
        self.assertIn("Generated by Roadrunner", out)
    ...
    def test_resync_replaces_block_not_duplicates(self):
        ...
        self.assertEqual(second.count("Birds seen during activity:"), 1)
        self.assertEqual(second.count("Generated by Roadrunner"), 1)
        self.assertEqual(second.count(matching.ROADRUNNER_URL), 1)  # URL not duplicated
